...
import TwoFact from '../component/TwoFact'

const Profile = () => {
  const session = useSession()
  const router = useRouter()
  const [profileBoxVisible, setProfileBoxVisible] = useState(false)
  const [two_step_status,set_two_step_status]=useState("https://i.ibb.co/9TCmzHv/off.png")
  const [show_tfa,set_show_tfa]=useState(false)
  ...
  const change_two_step=()=>{
    set_show_tfa(!show_tfa)
    if(two_step_status==="https://i.ibb.co/9TCmzHv/off.png"){
    set_two_step_status("https://i.ibb.co/BfRtfc5/on.png")
  }
  if(two_step_status==="https://i.ibb.co/BfRtfc5/on.png"){
    set_two_step_status("https://i.ibb.co/9TCmzHv/off.png")
  }
  }
  ...
  <img className="rounded-full border-[#B5C0D0] border-4" src={session.data.user.image} alt="avatar"/>
      ...
      {show_tfa && <TwoFact show_tfa={show_tfa} set_show_tfa={set_show_tfa} />}
      ...
      <div className="row-span-1 col-span-2 ... bg-[#F5E8DD] bg-opacity-30 border-[#3E3232] border-2 rounded-xl mt-2 ml-2 hover:shadow-2xl hover:drop-shadow-2xl transition-all duration-300 ease-in-out">
        ...
        <button onClick={change_two_step} className="mt-4"><img className="w-16" src={two_step_status} alt="2fa-off"></img></button>
        </div>
        </div>
        </div>
      </div>
      ...
      <div className="row-span-3 ... bg-[#CCD3CA] bg-opacity-30 border-[#3E3232] border-2 h-[90vh] rounded-xl mt-2 mb-2 mr-2 hover:shadow-2xl hover:drop-shadow-2xl transition-all duration-300 ease-in-out">
